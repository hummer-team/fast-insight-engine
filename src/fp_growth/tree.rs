...
impl<T: ItemType> Node<T> {
    ...
    /// Remove the given child node from this node's children list.
    /// No-op if the child is not found.
    pub fn remove_child(self: &Rc<Self>, child_node: RcNode<T>) {
        let mut children = self.children.borrow_mut();
        if let Some(index) = children.iter().position(|x| *x == child_node) {
...
impl<T: ItemType> Tree<T> {
    ...
    pub fn generate_partial_tree(paths: &[Vec<RcNode<T>>]) -> Tree<T> {
        ...
        let mut leaf_item: Option<T> = None;
        for path in paths.iter() {
            // Get the leaf item from the last node in this path.
            let last_node = match path.last() {
                Some(n) => n,
                None => continue,
            };
            let last_item = match last_node.item {
                Some(i) => i,
                None => continue,
            };
            leaf_item = Some(last_item);

            let mut cur_node = Rc::clone(&partial_tree.root_node.borrow());
            for path_node in path.iter() {
                let node_item = match path_node.item {
                    Some(i) => i,
                    None => continue,
                };
                match cur_node.search(node_item) {
                    Some(child_node) => {
                        cur_node = child_node;
                    }
                    None => {
                        let count = if path_node.item == leaf_item {
                            path_node.count.get()
                        } else {
                            0
                        };
                        let next_node = Node::new_rc(path_node.item, count);
        ...
        let leaf = match leaf_item {
            Some(item) => item,
            None => return partial_tree,
        };
        for path in partial_tree.generate_prefix_path(leaf).iter() {
            let leaf_count = match path.last() {
                Some(n) => n.count.get(),
                None => continue,
            };
            for path_node in path[..path.len().saturating_sub(1)].iter() {
    ...
    pub fn generate_prefix_path(&self, item: T) -> Vec<Vec<RcNode<T>>> {
        let route = match self.routes.get(&item) {
            Some(r) => r,
            None => return vec![],
        };
        let mut cur_end_node = Rc::clone(&route.0.borrow());
        let mut paths = vec![];
        loop {
    ...
    fn prune(&self) {
        ...
        for (item, nodes) in items_nodes.iter() {
            ...
            for node in nodes.iter() {
                ...
                while !cur_node.is_root() {
                    ...
                    match cur_node.parent() {
                        Some(parent_node) => cur_node = parent_node,
                        None => break,
                    }
                }
                path.push(cur_node);
            ...
            if let Some(ancestor) = common_ancestor {
                ancestor.add_child(Node::new_rc(Some(*item), leaf_node_count.iter().sum()));
            }
        }
    }
...
mod tests {
    ...
    /// Node creation: item and count are stored correctly.
    #[test]
    fn test_node_new_stores_item_and_count() {
        let node = Node::new(Some(42u32), 3);
        assert_eq!(node.item(), Some(42u32));
        assert_eq!(node.count(), 3);
        assert!(!node.is_root());
        assert!(node.is_leaf());
    }

    /// Root node: item=None, count=0.
    #[test]
    fn test_node_root_detection() {
        let root = Node::<u32>::new(None, 0);
        assert!(root.is_root());
    }

    /// Tree add_transaction: single transaction inserts nodes correctly.
    #[test]
    fn test_tree_add_single_transaction() {
        let mut tree = Tree::<u32>::new();
        tree.add_transaction(vec![1, 2, 3]);

        let nodes_1 = tree.get_all_nodes(1);
        assert_eq!(nodes_1.len(), 1);
        assert_eq!(nodes_1[0].count(), 1);

        let nodes_2 = tree.get_all_nodes(2);
        assert_eq!(nodes_2.len(), 1);
        assert_eq!(nodes_2[0].count(), 1);
    }

    /// Tree add_transaction: shared prefix increments counts.
    #[test]
    fn test_tree_shared_prefix_increments_count() {
        let mut tree = Tree::<u32>::new();
        tree.add_transaction(vec![1, 2]);
        tree.add_transaction(vec![1, 3]);

        // Item 1 is shared between both transactions.
        let nodes_1 = tree.get_all_nodes(1);
        assert_eq!(nodes_1.len(), 1, "item 1 should share one node");
        assert_eq!(nodes_1[0].count(), 2, "item 1 count should be 2");

        // Items 2 and 3 branch off separately.
        assert_eq!(tree.get_all_nodes(2).len(), 1);
        assert_eq!(tree.get_all_nodes(3).len(), 1);
    }

    /// generate_prefix_path: returns empty for unknown item.
    #[test]
    fn test_generate_prefix_path_unknown_item_returns_empty() {
        let tree = Tree::<u32>::new();
        let paths = tree.generate_prefix_path(99);
